...
class _SignInScreenState extends State<SignInScreen> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      final storedName = StorageService.prefs.getString('user_name');
      ...
      if (storedEmail == enteredEmail && storedPassword == enteredPassword) {
        ...
        // Check if user data exists, if not recreate it
        final hasUserData = await StorageService.hasAccount();
        if (!hasUserData) {
          // Recreate user data from stored credentials
  ...
  InputDecoration _inputDecoration(String label, IconData icon) =>
      InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(16),
          borderSide: const BorderSide(color: settingsColor, width: 2),
        ),
